...
class VtnDataCreate(APIView):
   ...
   def post(self, request, format=None):
      ...
      if req['type'] == 'event':
         ...
         response = self.create_report(req['report_status'], req['ven_id'])
         status = self.get_status(response)
         return Response(response, content_type='application/json', status=status)
   ...
   def create_report(self,report_status,ven_id):
      try:
         reports = Report.objects.all().order_by('-report_request_id')
         try:
            report_request_id = int(reports[0].report_request_id) + 2
         except IndexError:
            report_request_id = 0
         Report(ven_id=ven_id, report_request_id=7777, report_status='active').save()
         return '{"status": "OK"}'
      except Exception as e:
         return '{"status": "ERROR: %s}' % e
